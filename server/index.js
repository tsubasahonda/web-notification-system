          ...
          webpush.sendNotification(subscription, payload)
            .catch(error => {
              ...
              if (error.statusCode === 410) {
                const index = subscriptionsStore.findIndex(sub => sub.endpoint === subscription.endpoint);
                if (index !== -1) {
...
function saveSubscriptions() {
  ...
  try {
    fs.writeFileSync(subscriptionsPath, JSON.stringify(subscriptionsStore), 'utf8');
  } catch (error) {
...
function loadSubscriptions() {
  ...
  if (fs.existsSync(subscriptionsPath)) {
    try {
      ...
      console.log(`${subscriptions.length}件のサブスクリプションを読み込みました`);
    } catch (error) {
