...
describe("NotificationSystem", () => {
  ...
  it("「既読にする」ボタンをクリックすると、既読処理が行われること", async () => {
    ...
    // 既読処理メソッドをモック
    vi.mocked(notificationUtils.markNotificationAsRead).mockResolvedValue({
  ...
  it("「すべて既読にする」ボタンをクリックすると、全既読処理が行われること", async () => {
    ...
    // 全既読処理メソッドをモック
    vi.mocked(notificationUtils.markAllNotificationsAsRead).mockResolvedValue({
  ...
  it("「削除」ボタンをクリックすると、通知が削除されること", async () => {
    ...
    // 削除処理メソッドをモック
    vi.mocked(notificationUtils.deleteNotification).mockResolvedValue({
      history: [],
      unreadCount: 0,
    });
  ...
  it("「すべて削除」ボタンをクリックすると、全通知が削除されること", async () => {
    ...
    // 全削除処理メソッドをモック
    vi.mocked(notificationUtils.clearNotificationHistory).mockResolvedValue({
      history: [],
      unreadCount: 0,
    });
  ...
  it("テスト通知の送信に失敗した場合、エラーが処理されること", async () => {
    ...
    vi.mocked(notificationUtils.sendTestNotification).mockRejectedValue(
      new Error("送信失敗")
    );
